...
public class OptimUnit : MonoBehaviour
{
    ...
    void Update()
    {
        // Start profiling for "Handling Time"
        Profiler.BeginSample("Handling Time");
        HandleTime();
        Profiler.EndSample();

        // Start profiling for "Rotating"
        ...
        Profiler.EndSample(); // end profiling for "Rotating"

        // Start profiling for "Moving"
        Profiler.BeginSample("Moving");
        Move();
        Profiler.EndSample(); // end profiling for "Moving"

        // Start profiling for "Boundary Check"
        ...
        {
            currentVelocity.x *= -1;
            PickNewVelocityChangeTime(); // Pick a new change time as we changed velocity
        }
        ...
        Profiler.EndSample(); // end profiling for "Boundary Check"
    }
